...
def validate_fbx(fbx_path: Path) -> FBXValidationResult:
    ...
    # Step 4: Azure blendshape validation
    _validate_azure_blendshapes(result)
...
def _validate_azure_blendshapes(result: FBXValidationResult):
    """Validate that required Azure blendshapes are present in the FBX."""
    logger.step_start("Azure Blendshape Validation", "Checking for required Azure blendshapes")

    # Create a set of found blendshape names for faster lookup
    found_names_set = set(result.found_blendshapes)

    # Check for each required Azure blendshape
    missing_azure_blendshapes = []
    found_azure_blendshapes = []

    for azure_name in AZURE_BLENDSHAPES:
        # Check direct match first
        if azure_name in found_names_set:
            found_azure_blendshapes.append(azure_name)
            continue

        # Check for MetaHuman naming variations
        found_match = False
        for found_name in result.found_blendshapes:
            # Check if this found name maps to the Azure name
            if found_name in METAHUMAN_NAME_MAPPINGS and METAHUMAN_NAME_MAPPINGS[found_name] == azure_name:
                found_azure_blendshapes.append(azure_name)
                found_match = True
                break

        if not found_match:
            missing_azure_blendshapes.append(azure_name)

    # Log results
    found_count = len(found_azure_blendshapes)
    missing_count = len(missing_azure_blendshapes)

    if missing_count == 0:
        logger.validation_result("Azure blendshapes", True, f"All 52 Azure blendshapes found")
    else:
        logger.validation_result("Azure blendshapes", False, f"Missing {missing_count}/52 Azure blendshapes")
        result.add_warning(f"Missing Azure blendshapes: {', '.join(missing_azure_blendshapes[:5])}{'...' if missing_count > 5 else ''}")

    logger.console.print(f"  Azure blendshapes found: {found_count}/52")
    if missing_count > 0:
        logger.console.print(f"  Missing: {missing_count} blendshapes")
        # Show first few missing ones as examples
        examples = missing_azure_blendshapes[:3]
        logger.console.print(f"  Examples: {', '.join(examples)}")

    logger.step_complete("Azure Blendshape Validation")
